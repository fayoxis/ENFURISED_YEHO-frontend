<template>
    <div class="lg:max-w-6xl lg:mx-auto lg:p-10">
        <div class="bg-white rounded-[32px] p-8 relative shadow-lg">
            <!-- Close Button -->
            <button @click="$emit('close')"
                class="absolute top-6 right-6 text-2xl font-bold hover:opacity-70 transition-opacity">
                ×
            </button>

            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-6xl font-black tracking-wider mb-2">PLANIFIER VOTRE VISITE </h1>
                <div class="flex items-center gap-3">
                    <h2 class="text-2xl font-bold">APPARTEMENT MODERNE</h2>
                    <div class="flex items-center text-[#A2C02D] gap-1">
                        <img src="/icons/location-icon.svg" alt="location-icon" class="h-5" srcset="">
                        <span class="text-xl font-bold">MESSAMENDONGO, ODZA, YAOUNDÉ</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-row gap-4">
                ...
                <div class="calendar-section w-1/3">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div class="relative inline-block">
                            <button @click="toggleMonthSelect" class="flex items-center gap-2 text-xl font-bold">
                                {{ currentMonth }}
                            ...
                            <TransitionRoot appear :show="isMonthSelectOpen">
                                <div
                                    class="absolute top-full left-0 mt-1 bg-white shadow-lg rounded-lg py-2 z-50 min-w-[160px]">
                        ...
                        <span class="px-5 py-1 bg-[#D9F9E9] text-[#8DAD9D] rounded-3xl font-extrabold text-sm">
                            {{ currentYear }}
                        </span>
                    </div>
                    ...
                    <div class="mt-8 w-full flex items-center gap-4">
                        ...
                        <div class="flex gap-2">
                            <button @click="nextMonth"
                                class="w-10 h-10 rounded-full border-2 border-gray-300 flex items-center justify-center hover:bg-gray-100">
                                ▲
                            </button>
                        ...
                        <div class="w-full items-center">
                            <div class="relative">
                                <button @click="toggleTimeSelect"
                                    class="w-full flex items-center justify-between bg-white shadow-lg rounded-full px-6 py-3 hover:bg-gray-200 transition-colors">
                                ...
                                <TransitionRoot appear :show="isTimeSelectOpen">
                                    <div
                                        class="absolute top-full left-0 right-0 mt-2 bg-white shadow-lg rounded-xl py-2 z-50">
                ...
                <!-- Details Section -->
                <div class="appointment-details w-2/3 px-10">
                    <!-- Appointment Summary -->
                    <div class="mb-6">
                        <p class="text-black font-extrabold text-xl mb-1">Vous prenez rendez-vous pour le</p>
                        <p class="text-[#F24405] font-extrabold text-3xl">
                            {{ formatAppointmentDate() }}
                        </p>
                        <p class="text-[#F24405] font-extrabold text-3xl">en {{ selectedTime }}</p>
                    </div>

                    <!-- Information Text -->
                    <div class="text-xs text-[#8DAD9D] space-y-1 mb-8">
                        <p>• L'agent immobilier vous contactera pour fixer avec vous les détails exacts du rendez-vous.
                        </p>
                        ...
                        <label v-for="reminder in reminderOptions" :key="reminder.id"
                            ...
                            <div class="flex items-center gap-2">
                                <img :src="reminder.icon"
                                    :class="[reminder.icon, 'w-6 h-6 flex items-center justify-center']" />
                                <span
                                    class="text-xs font-extrabold leading-tight group-hover:text-[#A2C02D] transition-colors">
                                    {{ reminder.label }}
                                </span>
                            </div>
                        </label>
                    ...
                    <div class="gap-4 flex">
                        <button @click="validateAppointment"
                            class="w-full bg-[#A2C02D] hover:bg-[#7CB342] text-white py-4 rounded-xl relative overflow-hidden group transition-colors">
                            <div class="flex items-center justify-center">
                                <span class="font-medium text-sm">VALIDER LE RENDEZ-VOUS</span>
                                <span class="text-xs ml-1 opacity-80">PAIEMENT DES FRAIS DE VISITE</span>
                                <i class="fas fa-chevron-down ml-2 group-hover:translate-x-1 transition-transform"></i>
                            </div>
                        </button>

                        <div class="w-1/4 justify-start items-baseline">
                            <div class="space-x-3">
                                <span class="text-4xl font-extrabold text-[#F24405]">05</span>
                                <span class="text-sm font-extrabold text-[#F24405]">Pièces</span>
                            </div>
                            <div class="border border-[#8DAD9D] w-1/2"></div>
                            <div class="text-[#8DAD9D] flex flex-col text-sm ">
                                Votre Solde
                                <span class="font-extrabold text-xl">7 585 P.</span>
                            </div>
                        </div>
                    </div>
                </div>
